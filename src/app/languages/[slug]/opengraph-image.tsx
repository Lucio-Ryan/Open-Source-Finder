...
import { getLanguageBySlug } from '@/lib/mongodb/queries';

export const runtime = 'nodejs';
export const alt = 'Open Source Projects by Language';
...
export default async function Image({ params }: { params: { slug: string } }) {
  ...
  try {
    ...
    if (language) {
      ...
      totalStars = language.totalStars;
    }
  } catch (e) {
    // Use defaults
  }

  const starsText = totalStars >= 1000 ? `${(totalStars / 1000).toFixed(1)}k`.replace('.0k', 'k') : totalStars.toString();
    ...
    (
      <div
        style={{
          ...
          backgroundImage: 'radial-gradient(circle at 25% 75%, #1C1C1C 0%, transparent 50%)',
        }}
      >
        <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, zIndex: 10 }}>
          ...
          <div style={{ fontSize: '20px', color: '#60A5FA', fontWeight: 600, marginBottom: '12px', display: 'flex' }}>
            LANGUAGE
          </div>
          ...
          <div style={{ fontSize: '24px', color: '#A0A0A0', lineHeight: 1.4, marginBottom: '30px', maxWidth: '900px', display: 'flex' }}>
            Discover {count} open source projects built with {name}
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <div style={{
              ...
              <span style={{ fontSize: '18px', color: '#3ECF8E', fontWeight: 600 }}>{count} projects</span>
            </div>
            {totalStars > 0 && (
              <div style={{
                display: 'flex', alignItems: 'center', padding: '10px 20px', borderRadius: '999px',
                ...
                <span style={{ fontSize: '18px', color: '#FACC15', fontWeight: 600 }}>★ {starsText} total stars</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 10 }}>
          <span style={{ fontSize: '18px', color: '#666' }}>opensrc.me/languages/{params.slug}</span>
        </div>
      </div>
    ),
    { ...size }
